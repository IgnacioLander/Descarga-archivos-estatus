...
def run(playwright: Playwright) -> None:
    ...
    time.sleep(5)
    page.get_by_placeholder("Escribe aquí para buscar...").fill("cuidado bucal")
    page.get_by_role("link", name="Buscar").click()
    page.get_by_role("link", name="Programa Cuidado Bucal", exact=True).click()
    page.locator("#courses").click()
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    page.locator("div").filter(has_text=re.compile(r"^Prueba 2Obligatorio$")).locator("div").first.click()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "cuidado bucal")
 # ---------------------
