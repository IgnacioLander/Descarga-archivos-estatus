...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_placeholder("Escribe aquí para buscar...").fill("complementos")
    page.get_by_role("link", name="Buscar").click()
    page.get_by_role("link", name=" Programa Complementos de Salud | Escuela Comercial").click()
    time.sleep(2)
    page.locator("div#courses").click()
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    page.locator("div").filter(has_text=re.compile(r"^Prueba 2Obligatorio$")).get_by_role("checkbox").uncheck()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "complementos")
