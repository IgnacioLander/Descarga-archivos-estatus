...
def run(playwright: Playwright) -> None:
    ...
    time.sleep(3)
    page.get_by_placeholder("Escribe aquí para buscar...").fill("inspección")
    ...
    page.locator("#report-column-dropdown-container div").filter(has_text="País").nth(1).click()
    page.locator("input[name=\"UserStatus\"]").check()
    page.locator("input[name=\"EnrolledAs\"]").check()
    page.locator("input[name=\"Deleted\"]").uncheck()
    page.locator("#report-column-dropdown-container div").filter(has_text="Fecha de inscripción").nth(1).click()
    ...
    page.locator("#report-column-dropdown-container div").filter(has_text="Asistencia a clases").nth(1).click()
    page.locator("#report-column-dropdown-container div").filter(has_text="Visitas al curso").nth(1).click()
    page.locator("#report-column-dropdown-container div").filter(has_text="Horas en el curso").nth(1).click()
    page.locator("input[name=\"CompletedContentCount\"]").uncheck()
    page.get_by_role("link", name="Aplicar").click()
    page.locator("#user-activity-report div").filter(has_text="Exportar a excel").nth(4).click()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "inspección")
