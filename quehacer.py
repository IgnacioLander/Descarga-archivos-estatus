...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_placeholder("Escribe aquí para buscar...").fill("  Qué hacer y qué no hacer en el servicio al cliente - Escuela de Servicio  ")
    page.get_by_role("link", name="Buscar").click()
    page.locator("div").filter(has_text=re.compile(r"^Qué hacer y qué no hacer en el servicio al cliente - Escuela de Servicio$")).dblclick()
    time.sleep(2)
    page.locator("#courses").click()
    page.get_by_title("Select/Deselect All").click()
    ...
    page.locator("#report-column-dropdown-container div").filter(has_text="Primer acceso").nth(1).click()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "Que hacer")
