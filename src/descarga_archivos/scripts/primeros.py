...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_placeholder("Escribe aquí para buscar...").fill("primeros auxilios")
    page.get_by_role("link", name="Buscar").click()
    time.sleep(3)
    page.get_by_role("link", name="Programa Primeros Auxilios OTC y RX | Escuela Comercial").click()
    time.sleep(3)
    page.locator("#courses").click()
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    page.locator("div").filter(has_text=re.compile(r"^Introducción Programa Primeros Auxilios OTC y RXOptativo$")).get_by_role("checkbox").uncheck()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "PA")
