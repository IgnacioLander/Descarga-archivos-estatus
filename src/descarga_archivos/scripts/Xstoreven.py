...
def run(playwright: Playwright) -> None:
    ...
    time.sleep(3)
    page.get_by_placeholder("Escribe aquí para buscar...").fill("xstore")
    page.get_by_role("link", name="Buscar").click()
    page.locator("div:nth-child(10) > div > input").check()
    page.locator("div:nth-child(9) > div > input").check()
    page.locator("div:nth-child(7) > div > input").check()
    page.locator(".grid-canvas > div:nth-child(6) > div > input").check()
    page.locator("div").filter(has_text=re.compile(r"^Entrenamiento Key Users: XstoreCursoPrivada50Activo100%22/08\/202313\/09\/2023$")).get_by_role("checkbox").uncheck()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "Xstore")
