...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_role("link", name="Programa Tendencias de Cosméticos | Escuela").click()
    page.locator("#courses").click()
    page.locator("#pagerDropdown_grid-container_CareerCourses span").click()
    page.get_by_text("50").click()
    time.sleep(1)
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    time.sleep(1)
    page.locator("div").filter(has_text=re.compile(r"^Curso 2Obligatorio$")).get_by_role("checkbox").uncheck()
    page.locator("div").filter(has_text=re.compile(r"^Curso 3Obligatorio$")).get_by_role("checkbox").uncheck()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "Tendencias")
