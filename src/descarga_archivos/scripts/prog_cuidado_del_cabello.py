...
def run(playwright: Playwright) -> None:
    ...
    page.get_by_placeholder("Escribe aquí para buscar...").fill("cuidado del cabello")
    page.get_by_role("link", name="Buscar").click()
    page.get_by_role("link", name="Programa Cuidado del Cabello").click()
    time.sleep(1)
    page.locator("#courses").click()
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    page.locator("div").filter(has_text=re.compile(r"^Master Class \"Cuidado del Cabello\"Optativo$")).locator("div").first.click()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "Cuidado del Cabello")
