...
def run(playwright: Playwright) -> None:
    ...
    time.sleep(3)
    page.get_by_placeholder("Escribe aquí para buscar...").fill("nutrici")
    page.get_by_role("link", name="Buscar").click()
    page.get_by_role("link", name="Programa Nutrición | Escuela").click()
    page.locator("#courses").click()
    page.get_by_title("Select/Deselect All").get_by_role("checkbox").check()
    page.locator("div").filter(has_text=re.compile(r"^Live \"Mejorando la Asesoría en Dieta y Salud\"Optativo$")).get_by_role("checkbox").uncheck()
    page.locator("div").filter(has_text=re.compile(r"^Introducción a NutriciónObligatorio$")).get_by_role("checkbox").uncheck()
    ...
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "Nutrición")
