...
def run(playwright: Playwright) -> None:
    ...
    zip_file_path = os.path.join(download_path, "control.zip")
    ...
    time.sleep(3)
    page.get_by_placeholder("Escribe aquí para buscar...").fill("control de ingresos")
    ...
    for col in ["Deleted","EnrollmentDate"]:
    ...
    for col in ["LastAccessDate","GraduationDate","Satisfaction","Attendance","CourseAccessCount","TimeOnCourse","CompletedContentCount"]:
        page.locator(f"input[name=\"{col}\"]").uncheck()
    ...
    # Extract & rename
    df = descomprimir_y_leer_excel(zip_file_path, download_path, "control")
    print(df)
