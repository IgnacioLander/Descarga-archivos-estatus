...
def run(playwright: Playwright) -> None:
    ...
    browser = playwright.chromium.launch(headless=True)  # HEADLESS PARA GITHUB ACTIONS
    ...
    try:
        ...
        page.wait_for_selector("textbox[name='Email / NUMERO IDENTIDAD']", timeout=30_000)
        ...
        page.wait_for_selector("#manageIcon", timeout=30_000)
        page.click("#manageIcon")

        # 2. Buscar curso
        page.wait_for_selector("#ctl00_TopMenuControl_TopMenuDesktopControl1_adminOrTeacherSearch", timeout=20_000)
        ...
        # 3. Crear reporte
        page.wait_for_selector("#courses", timeout=30_000)
        page.click("#courses")
        ...
        # ⚠️ CRÍTICO: Esperar selector antes de continuar
        page.wait_for_selector("#popup-new-report-useractivity-report-icon", timeout=20_000)
        page.click("#popup-new-report-useractivity-report-icon")

        page.wait_for_selector("#column-date-filter_selectedArea", timeout=20_000)
        ...
        with page.expect_download(timeout=60_000) as download_info:
            page.wait_for_selector("a:has-text('Exportar a excel')", timeout=20_000)
            page.click("a:has-text('Exportar a excel')")

        download = download_info.value
        download.save_as(str(zip_file))

        # 6. Extraer y renombrar Excel
        ...
        debug_path.mkdir(exist_ok=True)
        (debug_path / "timeout.png").write_bytes(page.screenshot(full_page=True))
        (debug_path / "timeout.html").write_text(page.content(), encoding="utf-8")
        raise

    except Exception as e:
        print("Error general:", e)
        (debug_path / "error.png").write_bytes(page.screenshot(full_page=True))
        (debug_path / "error.html").write_text(page.content(), encoding="utf-8")
        raise
